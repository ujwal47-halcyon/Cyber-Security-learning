...
# Layer 1 — Physical
-> contains Cables, signals, connectors. Bits moving as electricity or radio waves.
No protocols here, just hardware.

- What breaks: Damaged cables, interference, faulty NICs.
- How to debug: Loopback test, swap cables, replace the network card.
...
=> Key concept: MAC addresses are hardware identifiers burned into NICs.
- The Data Link layer uses them to deliver frames within a local network.

Pentesting angle: ARP spoofing lives here. You can poison the ARP cache to
...
- What breaks: Wrong IP config, bad subnet masks, routing table issues.
...
- UDP — fast, no confirmation, used where speed matters (video, DNS, VoIP)
...
- Pentesting angle: SSL stripping attacks downgrade HTTPS to HTTP, removing
...
tells me what tools to use, what to look for in traffic, and where a
defensive control might stop it. The OSI model isn't just theory, 
it's a map for understanding what's actually happening during an engagement.
